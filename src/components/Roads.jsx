...
const Roads = ({ roads }) => {
  ...
  useEffect(() => {
    if (roads.length > 0) {
      let tempDeadEndEntities = roads.filter((entity) => entity.buildingModel === 'roadEnd');
      setDeadEndEntities(tempDeadEndEntities);
      let tempStraightEntities = roads.filter((entity) => entity.buildingModel === 'roadStraight');
      setStraightEntities(tempStraightEntities);
      let tempCurveEntities = roads.filter((entity) => entity.buildingModel === 'roadCurve');
      setCurveEntities(tempCurveEntities);
      let tempThreeWayEntities = roads.filter((entity) => entity.buildingModel === 'roadThreeWay');
      setThreeWayEntities(tempThreeWayEntities);
      let tempFourWayEntities = roads.filter((entity) => entity.buildingModel === 'roadFourWay');
